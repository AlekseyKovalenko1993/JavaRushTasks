...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable{
            int id;
            protected DogPet(int i){
                id = i;
            }
            public String getName(){
                StringBuilder str = new StringBuilder(getSuperQuotes());
                str.append(Dog.this.name);
                str.append(getSuperQuotes());
                return str.toString();
            }
            @Override
            public String say() {
                if(id < 1){
                    ...
                    StringBuilder str = new StringBuilder(getName() + " лает г");
                    for(int i = 0; i < id; i++){
                        str.append("а");
                    }
                    str.append("в! ");
                    str.append(this.formatter.format(new Date()));
                    return str.toString();
                }
            }
        }
